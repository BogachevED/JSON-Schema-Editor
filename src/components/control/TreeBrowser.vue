		...
		<Dialog id="FileGoTrashDialog" header="Отправка файла в коризну" v-model:visible="diaplayGoTrashDialog" style="border-radius: 8px" :style="{width: '50vw'}" :position="position" :modal="true">
			<template #footer>
				<table width="100%" border="0" cellpadding="5">
					<tr>
						<td width="90%"><div id="TextQuestionToGoTrahs" align="center" style="border-radius: 20px">Вы действительно хотите отправить файл "<font v-if="TreeNode.title != undefined" id="ChouseFile">{{TreeNode.title}}</font>" в корзину?</div></td>
						<td width="10%"></td>
					</tr>
					<tr>
						<td width="90%">
							<table width="100%" border="0" cellpadding="5">
								<tr>
									<td width="50%"><Button id="YesTrash" ref="YesTrash" label="Да" @click="YesGoTrash()" class="p-mr-2"/></td>
									<td width="50%"><Button id="NoTrash" ref="NoTrash" label="Нет" @click="NoGoTrash()" class="p-mr-2"/></td>
								</tr>
							</table>
						</td>
						<td width="10%">
...
export default {
	...
	methods: {
		...
		YesGoTrash () {
			...
			setTimeout(() => { this.diaplayGoTrashDialog = false }, 1000)
			setTimeout(() => { document.getElementById('YesTrash').removeAttribute('disabled') }, 1000)
			setTimeout(() => { document.getElementById('NoTrash').removeAttribute('disabled') }, 1000)
